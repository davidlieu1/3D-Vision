...
def reconstruct(imprefixCL, imprefixCR, imprefixL,imprefixR,threshold, cthresh, camL,camR):
	"""
	...
	pts3 : 2D numpy.array (dtype=float)
	"""
	maskCL = decode_color(imprefixCL, cthresh)
	maskCR = decode_color(imprefixCR, cthresh)
...
def decode_color(imprefix,threshold):
	bg = plt.imread(imprefix+'00.png').sum(axis = 2)
	fg = plt.imread(imprefix+'01.png').sum(axis = 2)
	(h,w) = bg.shape
	mask = np.ones((h,w))
	mask = mask*((np.abs(bg-fg))>threshold)
